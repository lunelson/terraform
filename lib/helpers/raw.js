...
/**
 * This is where we use harp.json to get some configs
 * These are then merged against defaults
 */
try {
  var harpConfig = exports.harpConfig = require(path.join(process.cwd(),'./harp.json'));
} catch(e) {
  try {
    var harpConfig = exports.harpConfig = require(path.join(process.cwd(),'./_harp.json'));
  } catch(e) {
    var harpConfig = exports.harpConfig = {};
  }
}
 ...
 * ...!!! but ONLY if browserify is true, in the harpConfig
 */
exports.needsBrowserify = function(source) {
	return harpConfig.browserify
    && /^[^#\/'"*]*(require|module|exports)\b/m.test(source)
